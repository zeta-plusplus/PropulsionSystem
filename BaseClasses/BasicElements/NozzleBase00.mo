...
partial model NozzleBase00
  ...
  ********************************************************/
  ...
  type switch_calcFg = enumeration(FullyExpandedFlow "", NozzleExitFlow "") annotation(
    Documentation(info = "<html>
    </html>"));
  //********** Parameters **********
  //----- switches -----
  parameter switch_calcFg switchCalc_Fg = switch_calcFg.NozzleExitFlow "" annotation(
    Dialog(group = "switch"),
    choicesAllMatching = true,
    Evaluate = true,
    HideResult = true);
  //********** Internal variables **********
  Medium.BaseProperties fluid_2Tot(p.start = p2Tot_init, T.start = T2Tot_init, state.p.start = p2Tot_init, state.T.start = T2Tot_init, h.start = h2Tot_init) "flow station of outlet, total";
  ...
  Real MNth;
  Medium.BaseProperties fluidStat_th(p.start = pStat_th_init, T.start = TStat_th_init, state.p.start = pStat_th_init, state.T.start = TStat_th_init, h.start = hStat_th_init) "static flow station of throat";
  Medium.BaseProperties fluid_th(p.start = p_th_init, T.start = T_th_init, state.p.start = p_th_init, state.T.start = T_th_init, h.start = h_th_init) "flow station of throat";
  //-----
  //--full expansion--
  Modelica.SIunits.Velocity V_th_fullExp(start = 0.0);
  Medium.BaseProperties fluidStat_th_fullExp(p.start = pStat_th_fullExp_init, T.start = TStat_th_fullExp_init, state.p.start = pStat_th_fullExp_init, state.T.start = TStat_th_fullExp_init, h.start = hStat_th_fullExp_init) "";
  //--choked states--
  Modelica.SIunits.Velocity V_th_choked(start = 0.0);
  Medium.BaseProperties fluidStat_th_choked(p.start = pStat_th_choked_init, T.start = TStat_th_choked_init, state.p.start = pStat_th_choked_init, state.T.start = TStat_th_choked_init, h.start = hStat_th_choked_init) "";
  //********** Interfaces **********
  ...
  //********** Initialization Parameters **********
  //--- fluid_2Tot ---
  parameter Modelica.SIunits.Pressure p2Tot_init(displayUnit = "Pa") = p1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.Temperature T2Tot_init(displayUnit = "K") = T1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.SpecificEnthalpy h2Tot_init(displayUnit = "J/kg") = h1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  //--- fluidStat_th ---
  parameter Modelica.SIunits.Pressure pStat_th_init(displayUnit = "Pa") = p2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.Temperature TStat_th_init(displayUnit = "K") = T2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.SpecificEnthalpy hStat_th_init(displayUnit = "J/kg") = h2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  //--- fluidStat_th_fullExp ---
  parameter Modelica.SIunits.Pressure pStat_th_fullExp_init(displayUnit = "Pa") = p2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.Temperature TStat_th_fullExp_init(displayUnit = "K") = T2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.SpecificEnthalpy hStat_th_fullExp_init(displayUnit = "J/kg") = h2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  //--- fluidStat_th_choked ---
  parameter Modelica.SIunits.Pressure pStat_th_choked_init(displayUnit = "Pa") = p2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.Temperature TStat_th_choked_init(displayUnit = "K") = T2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.SpecificEnthalpy hStat_th_choked_init(displayUnit = "J/kg") = h2_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  //--- fluid_th ---
  parameter Modelica.SIunits.Pressure p_th_init(displayUnit = "Pa") = p1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.Temperature T_th_init(displayUnit = "K") = T1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
  parameter Modelica.SIunits.SpecificEnthalpy h_th_init(displayUnit = "J/kg") = h1_init "" annotation(
    Dialog(tab = "Initialization", group = "Fluid states"));
equation
//********** Geometries **********
//##### none #####
