...
model Turboshaft_ex01
  ...
  Modelica.Blocks.Sources.Ramp ramp_NmechProp(duration = 10, height = 0, offset = 3000, startTime = 10) annotation(
    Placement(visible = true, transformation(origin = {-210, -80}, extent = {{10, 10}, {-10, -10}}, rotation = 180)));
  Modelica.Blocks.Math.UnitConversions.To_rpm to_rpm annotation(
    Placement(visible = true, transformation(origin = {-20, -130}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
...
equation
  ...
  connect(pressureRatio1.p_ratio, constraintDesPt.u_variable) annotation(
    Line(points = {{154, 26}, {154, 32}, {150, 32}, {150, 40}}, color = {0, 0, 127}));
  connect(speed.flange, powerGenerated.flange_b) annotation(
    Line(points = {{-120, -80}, {-100, -80}}));
  connect(powerGenerated.flange_a, speedSensor.flange) annotation(
    Line(points = {{-60, -80}, {-20, -80}}));
  annotation(
