...
model StirlingTypeAlpha_ex03
  ...
  Modelica_DeviceDrivers.Blocks.InputDevices.KeyboardInput keyboardInput1(sampleTime = 0.1)  annotation(
    Placement(visible = true, transformation(origin = {-320, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  InteractiveSimulation.Input.AccumulateDigitalInputSignal01 Accumulate(dt = 0.1, scl = 20, varInit = 288.15 + 100)  annotation(
    Placement(visible = true, transformation(origin = {-280, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  InteractiveSimulation.Input.AccumulateDigitalInputSignal01 Accumulate1(scl = -20)  annotation(
  ...
  InteractiveSimulation.Output.dispVarPyConsole02_00 dispVarPyConsole(nVariables = 4, nameVariables = {"time[s]", "Tsource[K]", "N[rpm]", "pwr[W]"})  annotation(
    Placement(visible = true, transformation(origin = {-310, -250}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  InteractiveSimulation.Utilities.TimeSignalOut timeSignalOut1 annotation(
    Placement(visible = true, transformation(origin = {-290, -170}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Mechanics.Rotational.Sensors.SpeedSensor speedSensor1 annotation(
    Placement(visible = true, transformation(origin = {-30, -210}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
...
equation
  connect(powerSensor1.power, dispVarPyConsole.u_variables[4]) annotation(
    Line(points = {{-18, -210}, {-18, -210}, {-18, -250}, {-298, -250}, {-298, -250}}, color = {0, 0, 127}));
  connect(to_rpm1.y, dispVarPyConsole.u_variables[3]) annotation(
    Line(points = {{-62, -230}, {-92, -230}, {-92, -250}, {-298, -250}, {-298, -250}}, color = {0, 0, 127}));
  connect(speedSensor1.w, to_rpm1.u) annotation(
    Line(points = {{-30, -222}, {-30, -222}, {-30, -230}, {-38, -230}, {-38, -230}}, color = {0, 0, 127}));
  connect(add1.y, dispVarPyConsole.u_variables[2]) annotation(
    Line(points = {{-228, 30}, {-222, 30}, {-222, -250}, {-299, -250}}, color = {0, 0, 127}));
  connect(timeSignalOut1.y, dispVarPyConsole.u_variables[1]) annotation(
    Line(points = {{-290, -182}, {-290, -250}, {-299, -250}}, color = {0, 0, 127}));
  connect(powerSensor1.flange_b, inertia2.flange_b) annotation(
    Line(points = {{0, -200}, {10, -200}}));
  connect(speedSensor1.flange, powerSensor1.flange_a) annotation(
    Line(points = {{-30, -200}, {-20, -200}}));
  connect(LossRotMech.flange_1, speedSensor1.flange) annotation(
    Line(points = {{-40, -200}, {-30, -200}}));
  connect(add1.y, hotHeatSource.T) annotation(
    Line(points = {{-229, 30}, {-204, 30}, {-204, 120}, {-142, 120}}, color = {0, 0, 127}));
  connect(Accumulate1.y, add1.u2) annotation(
    Line(points = {{-268, 10}, {-262, 10}, {-262, 24}, {-252, 24}}, color = {0, 0, 127}));
  connect(Accumulate.y, add1.u1) annotation(
    Line(points = {{-268, 40}, {-263, 40}, {-263, 36}, {-252, 36}}, color = {0, 0, 127}));
  connect(keyboardInput1.keyDown, Accumulate1.u) annotation(
    Line(points = {{-320, 18}, {-320, 18}, {-320, 10}, {-292, 10}, {-292, 10}}, color = {255, 0, 255}));
  connect(keyboardInput1.keyUp, Accumulate.u) annotation(
    Line(points = {{-309, 36}, {-303, 36}, {-303, 40}, {-293, 40}, {-293, 40}}, color = {255, 0, 255}));
  ...
  annotation(
    Diagram(coordinateSystem(extent = {{-340, -260}, {160, 180}})),
