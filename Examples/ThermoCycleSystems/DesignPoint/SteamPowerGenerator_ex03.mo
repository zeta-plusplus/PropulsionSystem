...
model SteamPowerGenerator_ex03 ""
  ...
  Modelica.Fluid.Sensors.Temperature T_evaporatorOutlet(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {96, 15}, extent = {{5, 5}, {-5, -5}}, rotation = 180)));
  Modelica.Fluid.Sensors.Pressure p_evaporatorOutlet(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {125, 5}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Fluid.Valves.ValveLinear VaporValve(redeclare package Medium = Modelica.Media.Water.StandardWater, dp_nominal = 10 * 1000, m_flow_nominal = 1) annotation(
    Placement(visible = true, transformation(extent = {{60, -10}, {80, -30}}, rotation = 0)));
  ...
  Modelica.Blocks.Math.Gain fracLiquid(k = 1 / evaporator.V_t) annotation(
    Placement(visible = true, transformation(origin = {-72, 15}, extent = {{-5, -5}, {5, 5}}, rotation = 90)));
  ...
  Modelica.Mechanics.Rotational.Sources.ConstantSpeed constantSpeed(w_fixed = 3000 * 2.0 * Modelica.Constants.pi / 60.0) annotation(
    Placement(visible = true, transformation(origin = {245, -80}, extent = {{5, -5}, {-5, 5}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sensors.PowerSensor powerTrb annotation(
    Placement(visible = true, transformation(origin = {225, -80}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ...
  Modelica.Mechanics.Rotational.Sources.ConstantSpeed constantSpeed1(w_fixed = 3000 * 2.0 * Modelica.Constants.pi / 60.0) annotation(
    Placement(visible = true, transformation(origin = {-195, -28}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Mechanics.Rotational.Sensors.PowerSensor powerPump annotation(
    Placement(visible = true, transformation(origin = {-175, -28}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ...
  Modelica.Blocks.Interaction.Show.RealValue realValue(significantDigits = 5, use_numberPort = true) annotation(
    Placement(visible = true, transformation(origin = {-133, -47}, extent = {{-13, -7}, {13, 7}}, rotation = 0)));
  Modelica.Blocks.Math.Gain gain(k = 1 / 1000) annotation(
    Placement(visible = true, transformation(origin = {-169, -47}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ...
  Modelica.Fluid.Sensors.Temperature temperature(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {205, -160}, extent = {{5, 5}, {-5, -5}}, rotation = 90)));
  Modelica.Fluid.Sensors.SpecificEnthalpy specificEnthalpy1(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {205, -135}, extent = {{-5, -5}, {5, 5}}, rotation = -90)));
  ...
  Modelica.Fluid.Vessels.ClosedVolume volume(redeclare package Medium = Modelica.Media.Water.StandardWater,V = 1.0, energyDynamics = Modelica.Fluid.Types.Dynamics.SteadyState, massDynamics = Modelica.Fluid.Types.Dynamics.SteadyState, nPorts = 2, use_HeatTransfer = true, use_portsData = false) annotation(
    Placement(visible = true, transformation(origin = {5, -35}, extent = {{-15, 15}, {15, -15}}, rotation = 0)));
  Modelica.Fluid.Sensors.TemperatureTwoPort T_evaporator_out(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {-40, -20}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
  Modelica.Blocks.Sources.Ramp ramp_Q_flow_in1(duration = 10, height = 1e6, offset = 5e6, startTime = 150) annotation(
...
equation
  ...
  connect(ctrl_pump_pi.y, constraint.u_targetValue) annotation(
    Line(points = {{-141, 50}, {-180, 50}, {-180, 42}}, color = {0, 0, 127}));
  connect(pump.u_PR, ramp_PR_pump.y) annotation(
    Line(points = {{-138, -36}, {-138, -79}}, color = {0, 0, 127}));
  connect(massFlowRate.port_b, pump.port_1) annotation(
    Line(points = {{-170, 0}, {-140, 0}, {-140, -20}}, color = {0, 127, 255}));
  connect(gain.y, realValue.numberPort) annotation(
    Line(points = {{-163.5, -47}, {-148, -47}}, color = {0, 0, 127}));
  ...
  connect(powerPump.power, gain.u) annotation(
    Line(points = {{-179, -33.5}, {-179, -47}, {-175, -47}}, color = {0, 0, 127}));
  connect(powerTrb.power, gain1.u) annotation(
    Line(points = {{221, -85.5}, {221, -107}, {187, -107}}, color = {0, 0, 127}));
  connect(trb.port_2, specificEnthalpy1.port) annotation(
    Line(points = {{172, -64}, {200, -64}, {200, -134}}, color = {0, 127, 255}));
  connect(specificEnthalpy1.port, temperature.port) annotation(
    Line(points = {{200, -135}, {200, -161}}, color = {0, 127, 255}));
  connect(Condenser.port_b, temperature.port) annotation(
    Line(points = {{100, -160}, {200, -160}}, color = {0, 127, 255}));
  ...
  connect(massFlowVapor.m_flow, constraint1.u_targetValue) annotation(
    Line(points = {{110, -31}, {110, -110}, {30, -110}, {30, -118}}, color = {0, 0, 127}));
  ...
  connect(T_heater_out.port_b, VaporValve.port_a) annotation(
    Line(points = {{50, -20}, {60, -20}}, color = {0, 127, 255}));
  connect(evaporator.port_b, T_evaporator_out.port_a) annotation(
    Line(points = {{-60, -20}, {-50, -20}}, color = {0, 127, 255}));
  connect(T_evaporator_out.port_b, volume.ports[1]) annotation(
    Line(points = {{-30, -20}, {6, -20}}, color = {0, 127, 255}));
  connect(volume.ports[2], T_heater_out.port_a) annotation(
    Line(points = {{6, -20}, {30, -20}}, color = {0, 127, 255}));
  connect(heatSupply1.port, volume.heatPort) annotation(
    Line(points = {{-10, -58}, {-10, -34}}, color = {191, 0, 0}));
  connect(ramp_Q_flow_in1.y, heatSupply1.Q_flow) annotation(
    Line(points = {{-10, -88}, {-10, -78}}, color = {0, 0, 127}));
  annotation(
